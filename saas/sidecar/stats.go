...
type storageStats struct {
	...
	WALBytes  string `json:"walBytes"`
	CDCBytes  string `json:"cdcBytes"`
}
...
func collectStorageStats(dir, name string) storageStats {
	return storageStats{
		DataBytes: decimalFileAllocatedBytes(filepath.Join(dir, name+".dat")),
		WALBytes:  decimalFileAllocatedBytes(filepath.Join(dir, name+".wal")),
		CDCBytes:  decimalFileAllocatedBytes(filepath.Join(dir, name+".cdc")),
	}
}

func decimalFileAllocatedBytes(path string) string {
	n, err := fileAllocatedBytes(path)
	if err != nil {
		return "0"
	}
