...
export const configSchema = z
  .object({
    ...
    fabricApi: z.string().url().default("https://api.fabric.microsoft.com"),
    gatewayUrl: z.string().url(),
    gatewayToken: z.string().min(1),
    notebookGatewayUrl: z.string().url().optional(),
    keyVaultUrl: z.string().url(),
    keyVaultSecretName: z.string().min(1).default("asmdb-gateway-token"),
    ...
    notebookCdcBytes: positiveInt.default(16 * 1024 * 1024),
    upstreamTimeoutMs: positiveInt.default(8000),
    fabricOperationTimeoutMs: positiveInt.default(60 * 1000),
    fabricOperationPollMs: positiveInt.default(1000),
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  return configSchema.parse({
    ...
    fabricApi: env.ASMDB_FABRIC_API,
    gatewayUrl: env.ASMDB_GATEWAY_URL,
    gatewayToken: env.ASMDB_GATEWAY_TOKEN,
    notebookGatewayUrl: env.ASMDB_NOTEBOOK_GATEWAY_URL,
    keyVaultUrl: env.ASMDB_KEY_VAULT_URL,
    keyVaultSecretName: env.ASMDB_KEY_VAULT_SECRET_NAME,
    ...
    notebookCdcBytes: env.ASMDB_NOTEBOOK_CDC_BYTES,
    upstreamTimeoutMs: env.ASMDB_UPSTREAM_TIMEOUT_MS,
    fabricOperationTimeoutMs: env.ASMDB_FABRIC_OPERATION_TIMEOUT_MS,
    fabricOperationPollMs: env.ASMDB_FABRIC_OPERATION_POLL_MS,
