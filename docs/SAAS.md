...
- **The engine is tiny and starts instantly.** `asmdb` is ~30 KB, has no
...
> native **ELF64 binary** (~39 KB) that depends on nothing but the kernel — no
...
- **Data plane**: one **micro-container per database instance**. The image is a
  `FROM scratch` bundle of the **Linux ELF64 engine** (~39 KB, zero shared-library
