...
1. **Engine:** the `asmdb` binary is assembled directly with NASM using
   ...
   package dependencies. Local builds use the NASM installed on the developer or
   test machine, so the NASM version is **not pinned**.
...
The machine-readable SBOM is `docs/sbom.json`. It uses a
...
When `mcp/package-lock.json` changes, regenerate `docs/sbom.json` from the lockfile
instead of editing dependency versions by hand. Do not invent versions or hashes:
if a tool version is not pinned by the repository, record it as
`not pinned`.
...
All transitive package versions are listed in `docs/sbom.json`.
